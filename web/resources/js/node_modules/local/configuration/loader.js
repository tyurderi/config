module.exports = Class.extend(EventAbstract, {
    ...
    load: function()
    {
        ...
        Querier.query('config', [ me.configId ]).with('columns').submit(function(response) {
    ...
    {
        var me    = this,
            query = Querier.query('table_load', [ me.configId ]).params({
    ...
    processRow: function(row)
    {
        ...
        $edit.on('click', function() {
            (new (require('local/configuration/form_loader'))(me.$container, me.config)).load(row.id);
        });

        $remove.on('click', function() {
            if(confirm('Are you sure to delete this row? You can not undo!'))
            {
                Querier.query('delete', [ me.configId, row.id ]).submit(function(response) {
                    if(response.success)
                    {
                        me.loadData();
                    }
